...
public class UserDataBeans implements Serializable{
    ...
    /**
     * フォーム入力が空(もしくは0)だったときそれらの情報を格納する配列
     * insert-insertconfirm,およびupdate-updateresultでの処理に用いる
     */
    ...
    /**
     * update画面のフォームに入力されるようにDTOの値をUserDataBeans型に変換する
     * @param udb　UserDataBeans
     * @param udd ResultDetail取得したUserDataDTO型の個人情報
     */
    public void DateFormat(UserDataBeans udb, UserDataDTO udd){
        SimpleDateFormat sdf =new SimpleDateFormat("yyyyMMdd");
        String birth=sdf.format(udd.getBirthday());
        udb.setYear(birth.substring(0,4));
        udb.setMonth(birth.substring(4,6));
        udb.setDay(birth.substring(6,8));
